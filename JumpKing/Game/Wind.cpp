...
void Wind::Update(float deltaTime)
{
    ...
    //
    // 1) _force = sinf(_phase) * GameConstants::WIND_AMPLITUDE 로 현재 바람 세기를 구한다.
    // 2) _phase += GameConstants::WIND_PHASE_SPEED * deltaTime 로 위상을 전진시킨다.
    ...
    _force = sinf(_phase) * GameConstants::WIND_AMPLITUDE;
    _phase += GameConstants::WIND_PHASE_SPEED * deltaTime;
